...
def given_dataloader_test(
    ...
    with torch.no_grad():
        for batch_idx, (x, target, *additional_info) in enumerate(test_dataloader):
            ...
            metrics['test_correct'] += (pred.argmax(-1) == target).sum().item()
...
def general_plot_for_epoch(
    ...
    for idx, (label, value_list) in enumerate(labelToListDict.items()):
        ...
        if validate_list_for_plot(value_list, len_of_first_valueList):
            plt.plot(range(len(value_list)), value_list, marker = marker_style, markersize = 13, linewidth = 4, label = label, linestyle = linestyle, markevery = 1,)
        else:
    ...
    leg = ax.legend(
        ...
        fontsize = 16,  # 글씨 크게
        frameon = True,
        borderpad = 1.2,  # 상자 안 여백
        labelspacing = 1.0,  # 줄 간격
        handlelength = 2.5,  # 선 핸들 길이
        handletextpad = 0.8,
        markerscale = 1.1  # 범례의 마커도 크게
    )
    if leg is not None:
...
def plot_loss(
    ...
    if validate_list_for_plot(train_loss_list, len_set):
        plt.plot(x, train_loss_list, marker="o", linewidth=4, label="Train Loss", linestyle="--", markersize=13, markevery=1,)
    else:
        logging.warning("train_loss_list contains None or len not match")
    if validate_list_for_plot(clean_test_loss_list, len_set):
        plt.plot(x, clean_test_loss_list, marker="v", linewidth=4, label="Test Clean loss", linestyle="-", markersize=13, markevery=1,)
    else:
        logging.warning("clean_test_loss_list contains None or len not match")
    if validate_list_for_plot(bd_test_loss_list, len_set):
        plt.plot(x, bd_test_loss_list, marker="+", linewidth=4, label="Test Backdoor Loss", linestyle="-.", markersize=13, markevery=1,)
    else:
...
def plot_acc_like_metric_pure(
    ...
    if validate_list_for_plot(train_acc_list, len_set):
        plt.plot(x, train_acc_list,marker="o",linewidth=4,label="Train Acc",linestyle="--",markersize=13, markevery=1,)
    else:
        logging.warning("train_acc_list contains None, or len not match")
    if validate_list_for_plot(test_acc_list, len_set):
        plt.plot(x, test_acc_list, marker="o",linewidth=4,label="Test C-Acc",linestyle="--", markersize=13, markevery=1,)
    else:
        logging.warning("test_acc_list contains None, or len not match")
    if validate_list_for_plot(test_asr_list, len_set):
        plt.plot(x, test_asr_list,  marker="v", linewidth=4, label="Test ASR", linestyle = "-", markersize=13, markevery=1,)
    else:
...
def plot_acc_like_metric(
    ...
    if validate_list_for_plot(train_asr_list, len_set):
        plt.plot(x, train_asr_list, marker="v", linewidth=4, label="Train ASR", linestyle="-", markersize=13, markevery=1)
    else:
...
class ModelTrainerCLS_v2():
    ...
    def one_forward_backward(self, x, labels, device, verbose=0):
        ...
        if verbose == 1:
            batch_predict = torch.max(out, -1)[1].detach().clone().cpu()
...
class PureCleanModelTrainer(ModelTrainerCLS_v2):
    ...
    def test_given_dataloader_on_mix(self, test_dataloader, device = None, verbose = 0):
        ...
        with torch.no_grad():
            for batch_idx, (x, labels, original_index, poison_indicator, original_targets) in enumerate(test_dataloader):
                ...
                functional.reset_net(self.model)  # 매 배치 상태 초기화

                if x.dim() == 5 and x.size(0) != 16:
                    x = x.transpose(0, 1)
                ...
                if pred.dim() == 3:
                    # 후보 A: [T,N,C]
                    if pred.size(0) == 16:
                        pred = pred.mean(0)  # [N,C]
                    # 후보 B: [N,C,T]
                    elif pred.size(-1) == 16:
                        pred = pred.mean(-1)  # [N,C]
                    else:
                        # 그 외 3차원: 나머지 축 평균
                        pred = pred.mean(0)
                elif pred.dim() > 3:
                    # [*,*,*,*]이면 (배치, 클래스)만 남기고 전부 평균
                    # 배치 축: 0, 클래스 축 후보: 1이나 -1. num_classes로 판별
                    if pred.size(1) == 10:
                        pred = pred.view(pred.size(0), pred.size(1), -1).mean(-1)  # [N,C]
                    elif pred.size(-1) == 10:
                        pred = pred.view(pred.size(0), -1, pred.size(-1)).mean(1)  # [N,C]
                    else:
                        # 마지막 안전장치: (배치, 나머지 전부)로 펴고 선형층 출력이라고 간주
                ...
                if batch_idx == 0 and verbose == 1:
                    print(f"[on_mix test] x:{tuple(x.shape)} pred:{tuple(pred.shape)} labels:{tuple(labels.shape)}")
    ...
    def train_with_test_each_epoch_on_mix(self,
        ...
        excel_path = os.path.join(save_folder_path, "finetune_log.xlsx")

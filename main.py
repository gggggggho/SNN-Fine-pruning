...
from util import loss_picker, optimizer_picker, backdoor_model_trainer, save_experiments, calculate_asr
...
def main():
    ...
    if args.epochs == 0 and args.model_path is not None:
        _, _, poison_testloader = create_backdoor_data_loader(args)
        asr = calculate_asr(model, poison_testloader, args.trigger_label, device)
        print(f"[RESULT] Attack Success Rate (ASR): {asr * 100:.2f}%")
        return

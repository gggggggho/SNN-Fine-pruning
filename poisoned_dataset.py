...
class PoisonedDataset(Dataset):
    ...
    def __init__(self, dataset, trigger_label=0, mode='train', epsilon=0.1, pos='top-left', attack_type='static', time_step=16,
        ...
        if not torch.is_tensor(self.targets):
            self.original_targets = torch.Tensor(self.targets).long()
        else:
            self.original_targets = self.targets.clone().long()
    ...
    def __getitem__(self, item):
        ...
        # 스위치가 켜져 있으면 원본 라벨, 꺼져 있으면 포이즌 라벨 사용
        if self.getitem_all_switch:
            lbl = self.original_targets[item]
        else:
            lbl = self.targets[item].long()

        # 학습 시(train)엔 one-hot, 테스트 시(test)엔 정수 레이블 반환
        if self.mode == 'train':
            return img, F.one_hot(lbl, self.class_num).float()
        else:
            return img, lbl
...
def create_backdoor_data_loader_adaptive(args):
    ...
    clean_trainloader = DataLoader(
        dataset=clean_trainset, batch_size=args.batch_size, shuffle=True, num_workers=0)
    bk_trainloader = DataLoader(
        dataset=bk_trainset, batch_size=args.batch_size, shuffle=True, num_workers=0)
    test_data_ori_loader = DataLoader(
        dataset=test_data_ori, batch_size=args.batch_size, shuffle=False, num_workers=0)
    test_data_tri_loader = DataLoader(
        dataset=test_data_tri, batch_size=args.batch_size, shuffle=False, num_workers=0)
